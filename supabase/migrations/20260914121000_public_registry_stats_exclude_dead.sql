-- Exclude dead animals from living-registry public stats (same idea as item legacyat).

DROP FUNCTION IF EXISTS public.public_registry_stats ();

CREATE OR REPLACE FUNCTION public.public_registry_stats ()
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  WITH
    days AS (
      SELECT (d::date) AS day
      FROM generate_series(
        (now() at time zone 'utc')::date - interval '13 days',
        (now() at time zone 'utc')::date,
        interval '1 day'
      ) AS d
    ),

    totals AS (
      SELECT
        (SELECT count(*)::int
         FROM public.users u
         WHERE u.deleted_at IS NULL
           AND u.suspended_at IS NULL
           AND u.disabled_at IS NULL) AS total_users,

        (SELECT count(*)::int
         FROM public.items i
         WHERE i.deletedat IS NULL
           AND i.legacyat IS NULL) AS total_items,

        (SELECT count(*)::int
         FROM public.items i
         WHERE i.deletedat IS NULL
           AND i.legacyat IS NULL
           AND i.reportedstolenat IS NULL) AS active_items,

        (SELECT count(*)::int
         FROM public.items i
         WHERE i.deletedat IS NULL
           AND i.legacyat IS NULL
           AND i.reportedstolenat IS NOT NULL) AS stolen_items,

        (SELECT count(*)::int
         FROM public.livestock_animals a
         WHERE a.deleted_at IS NULL
           AND a.status NOT IN ('deleted', 'dead')) AS total_animals,

        (SELECT count(*)::int
         FROM public.livestock_animals a
         WHERE a.deleted_at IS NULL
           AND a.status = 'missing') AS missing_animals
    ),

    category_breakdown AS (
      SELECT
        COALESCE(NULLIF(trim(i.category), ''), 'Uncategorized') AS category,
        count(*)::int AS count
      FROM public.items i
      WHERE i.deletedat IS NULL
        AND i.legacyat IS NULL
      GROUP BY 1
    ),

    stolen_category_breakdown AS (
      SELECT
        COALESCE(NULLIF(trim(i.category), ''), 'Uncategorized') AS category,
        count(*)::int AS count
      FROM public.items i
      WHERE i.deletedat IS NULL
        AND i.legacyat IS NULL
        AND i.reportedstolenat IS NOT NULL
      GROUP BY 1
    ),

    animal_type_breakdown AS (
      SELECT
        COALESCE(NULLIF(trim(t.label), ''), NULLIF(trim(a.type_code), ''), 'Unknown') AS type_label,
        count(*)::int AS count
      FROM public.livestock_animals a
      LEFT JOIN public.livestock_types t ON t.code = a.type_code
      WHERE a.deleted_at IS NULL
        AND a.status NOT IN ('deleted', 'dead')
      GROUP BY 1
    ),

    missing_type_breakdown AS (
      SELECT
        COALESCE(NULLIF(trim(t.label), ''), NULLIF(trim(a.type_code), ''), 'Unknown') AS type_label,
        count(*)::int AS count
      FROM public.livestock_animals a
      LEFT JOIN public.livestock_types t ON t.code = a.type_code
      WHERE a.deleted_at IS NULL
        AND a.status = 'missing'
      GROUP BY 1
    ),

    daily_item_trend AS (
      SELECT
        to_char(d.day, 'YYYY-MM-DD') AS date,
        COALESCE((
          SELECT count(*)::int
          FROM public.items i
          WHERE i.deletedat IS NULL
            AND i.legacyat IS NULL
            AND (i.createdon at time zone 'utc')::date = d.day
        ), 0) AS count
      FROM days d
      ORDER BY d.day
    ),

    daily_user_trend AS (
      SELECT
        to_char(d.day, 'YYYY-MM-DD') AS date,
        COALESCE((
          SELECT count(*)::int
          FROM public.users u
          WHERE u.deleted_at IS NULL
            AND u.suspended_at IS NULL
            AND u.disabled_at IS NULL
            AND (u.created_at at time zone 'utc')::date = d.day
        ), 0) AS count
      FROM days d
      ORDER BY d.day
    ),

    daily_stolen_trend AS (
      SELECT
        to_char(d.day, 'YYYY-MM-DD') AS date,
        COALESCE((
          SELECT count(*)::int
          FROM public.items i
          WHERE i.deletedat IS NULL
            AND i.legacyat IS NULL
            AND i.reportedstolenat IS NOT NULL
            AND (i.reportedstolenat at time zone 'utc')::date = d.day
        ), 0) AS count
      FROM days d
      ORDER BY d.day
    ),

    daily_active_trend AS (
      SELECT
        to_char(d.day, 'YYYY-MM-DD') AS date,
        COALESCE((
          SELECT count(*)::int
          FROM public.items i
          WHERE i.deletedat IS NULL
            AND i.legacyat IS NULL
            AND i.reportedstolenat IS NULL
            AND (i.createdon at time zone 'utc')::date = d.day
        ), 0) AS count
      FROM days d
      ORDER BY d.day
    ),

    daily_animal_trend AS (
      SELECT
        to_char(d.day, 'YYYY-MM-DD') AS date,
        COALESCE((
          SELECT count(*)::int
          FROM public.livestock_animals a
          WHERE a.deleted_at IS NULL
            AND a.status NOT IN ('deleted', 'dead')
            AND (a.created_at at time zone 'utc')::date = d.day
        ), 0) AS count
      FROM days d
      ORDER BY d.day
    ),

    daily_missing_trend AS (
      SELECT
        to_char(d.day, 'YYYY-MM-DD') AS date,
        COALESCE((
          SELECT count(*)::int
          FROM public.livestock_animals a
          WHERE a.deleted_at IS NULL
            AND a.status = 'missing'
            AND (a.updated_at at time zone 'utc')::date = d.day
        ), 0) AS count
      FROM days d
      ORDER BY d.day
    ),

    top_stolen_categories AS (
      SELECT
        COALESCE(NULLIF(trim(i.category), ''), 'Uncategorized') AS category,
        count(*)::int AS count
      FROM public.items i
      WHERE i.deletedat IS NULL
        AND i.legacyat IS NULL
        AND i.reportedstolenat IS NOT NULL
      GROUP BY 1
      ORDER BY count DESC, category ASC
      LIMIT 5
    ),

    top_stolen_villages AS (
      SELECT
        COALESCE(NULLIF(trim(i.village), ''), 'Unknown') AS village,
        count(*)::int AS count
      FROM public.items i
      WHERE i.deletedat IS NULL
        AND i.legacyat IS NULL
        AND i.reportedstolenat IS NOT NULL
      GROUP BY 1
      ORDER BY count DESC, village ASC
      LIMIT 5
    ),

    top_user_villages AS (
      SELECT
        COALESCE(NULLIF(trim(u.village), ''), 'Unknown') AS village,
        count(*)::int AS count
      FROM public.users u
      WHERE u.deleted_at IS NULL
        AND u.suspended_at IS NULL
        AND u.disabled_at IS NULL
      GROUP BY 1
      ORDER BY count DESC, village ASC
      LIMIT 5
    )

  SELECT jsonb_build_object(
    'totals', jsonb_build_object(
      'totalUsers', (SELECT total_users FROM totals),
      'totalItems', (SELECT total_items FROM totals),
      'activeItems', (SELECT active_items FROM totals),
      'stolenItems', (SELECT stolen_items FROM totals),
      'totalAnimals', (SELECT total_animals FROM totals),
      'missingAnimals', (SELECT missing_animals FROM totals)
    ),

    'categoryBreakdown', COALESCE((
      SELECT jsonb_object_agg(category, count)
      FROM category_breakdown
    ), '{}'::jsonb),

    'stolenCategoryBreakdown', COALESCE((
      SELECT jsonb_object_agg(category, count)
      FROM stolen_category_breakdown
    ), '{}'::jsonb),

    'animalTypeBreakdown', COALESCE((
      SELECT jsonb_object_agg(type_label, count)
      FROM animal_type_breakdown
    ), '{}'::jsonb),

    'missingTypeBreakdown', COALESCE((
      SELECT jsonb_object_agg(type_label, count)
      FROM missing_type_breakdown
    ), '{}'::jsonb),

    'dailyItemTrend', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('date', date, 'count', count) ORDER BY date)
      FROM daily_item_trend
    ), '[]'::jsonb),

    'dailyUserTrend', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('date', date, 'count', count) ORDER BY date)
      FROM daily_user_trend
    ), '[]'::jsonb),

    'dailyStolenTrend', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('date', date, 'count', count) ORDER BY date)
      FROM daily_stolen_trend
    ), '[]'::jsonb),

    'dailyActiveTrend', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('date', date, 'count', count) ORDER BY date)
      FROM daily_active_trend
    ), '[]'::jsonb),

    'dailyAnimalTrend', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('date', date, 'count', count) ORDER BY date)
      FROM daily_animal_trend
    ), '[]'::jsonb),

    'dailyMissingTrend', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('date', date, 'count', count) ORDER BY date)
      FROM daily_missing_trend
    ), '[]'::jsonb),

    'topStolenItems', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'category', category,
        'count', count
      ))
      FROM top_stolen_categories
    ), '[]'::jsonb),

    'topStolenVillages', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'village', village,
        'count', count
      ))
      FROM top_stolen_villages
    ), '[]'::jsonb),

    'topUserVillages', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        'village', village,
        'count', count
      ))
      FROM top_user_villages
    ), '[]'::jsonb)
  );
$$;

GRANT EXECUTE ON FUNCTION public.public_registry_stats () TO service_role;
