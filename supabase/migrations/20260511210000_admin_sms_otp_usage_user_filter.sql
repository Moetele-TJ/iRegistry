-- Optional per-user filter for SMS OTP usage stats (audit_logs.user_id).

drop function if exists public.admin_sms_otp_usage_stats(integer);

create or replace function public.admin_sms_otp_usage_stats(
...
as $$
  ...
  sent as (
    ...
    where event = 'OTP_SENT'
      ...
      and (p_user_id is null or user_id = p_user_id::text)
    group by 1
  ),
  failed as (
    select
...
revoke all on function public.admin_sms_otp_usage_stats(integer, uuid) from public;
grant execute on function public.admin_sms_otp_usage_stats(integer, uuid) to service_role;
