...
serve(async (req) => {
  ...
  try {
    ...
    const { data: users, error } = await supabase
      .from("users")
      .select(
        "id, created_at, first_name, last_name, id_number, phone, email, role, police_station, village, ward, date_of_birth, suspended_reason, suspended_at, disabled_reason, disabled_at, deleted_at, slug, user_credits(balance)",
      )
